...
class WaveletTransform(Operator):
    ...
    def __init__(self, dom, nscales, wbasis, mode, axes=None):
...
-dwt-max-level>`_ .
        ...
        self._axes = axes
        if axes is not None:
            if len(axes) != dom.ndim:
                raise ValueError('The length of the axes has to be equal to '
                                 'dimension. Axes len is {} and dimension '
                                 'is {}.'.format(len(axes), dom.ndim))
            else:
                self.size_list = coeff_size_list_axes(dom.shape, self.wbasis,
                                                      self.mode, self._axes)
...
class WaveletTransformInverse(Operator):
    ...
    def __init__(self, ran, nscales, wbasis, mode, axes=None):
        """Initialize a new instance.
...
-dwt-max-level>`_ .
        ...
        self._axes = axes
        if axes is not None:
            if len(axes) != ran.ndim:
                raise ValueError('The length of the axes has to be equal to '
                                 'dimension. Axes len is {} and dimension '
                                 'is {}.'.format(len(axes), ran.ndim))
            else:
                self.size_list = coeff_size_list_axes(ran.shape, self.wbasis,
                                                      self.mode, self._axes)
