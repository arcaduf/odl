...
class WaveletTransform(Operator):
    ...
    def __init__(self, dom, wbasis, mode, nscales=None, axes=None):
...
-dwt-max-level>`_ .
        ...
        if axes is not None:
            ...
            axes_counts = [axes.count(i) for i in range(dom.ndim)]
            ...
            self.size_list = coeff_size_list(dom.shape, self.wbasis,
            ...
            self.size_list = coeff_size_list(dom.shape, self.wbasis, self.mode,
                                             self.nscales, axes=None)

            multiplicity = {1: 1, 2: 3, 3: 7}
            ran_size = (np.prod(self.size_list[0]) +
                        sum(multiplicity[dom.ndim] * np.prod(shape)
...
-dwt-max-level>`_ .
        ...
        if axes is not None:
            ...
            axes_counts = [axes.count(i) for i in range(ran.ndim)]
            for i in range(len(axes_counts)):
                if axes_counts[i] > max_level:
                    raise ValueError('Wavelet transforms per axes cannot be '
                                     'performed more than maximum useful '
            ...
            self.size_list = coeff_size_list(ran.shape, self.wbasis,
                                             self.mode, nscales=None,
                                             axes=self.axes)
