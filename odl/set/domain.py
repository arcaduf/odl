...
class IntervalProd(Set):
    ...
    @property
    def begin(self):
        """Left interval boundaries."""
        return self._begin

    @property
    def end(self):
        """Right interval boundaries."""
        return self._end
    ...
    @property
    def volume(self):
        """n-dimensional volume of this interval product."""
    ...
    def min(self):
        """Minimum point of this interval product"""
        return self.begin

    def max(self):
        """Maximum point of this interval product"""
        return self.end

    def extent(self):
        """Vector of interval lengths per axis."""
        return self.max() - self.min()

    def element(self, inp=None):
        """Create an element of this interval product.
        ...
        if inp is None:
            ...
            raise TypeError('`inp` {!r} is not a valid element in {!r}'
                            ''.format(inp, self))
    ...
    def contains_set(self, other, atol=0.0):
        """Return True ``other`` is (almost) contained in this set.
    ...
    def collapse(self, indices, values):
        ...
        for axis, index in enumerate(indices):
            if not 0 < index <= self.ndim:
                raise IndexError('index {} out of range 0 --> {} in axis {}'
                                 ''.format(index, self.ndim - 1, axis))
    ...
    def corners(self, order='C'):
        ...
        corners : numpy.ndarray
            ...
            array is ``2^m * ndim``, where ``m`` is the number of
    ...
    def __getitem__(self, indices):
        ...
        >>> rbox[[0, 1, 0]]
        Rectangle([-1.0, 2.0, -1.0], [-0.5, 3.0, -0.5])
        """
    ...
    def __rdiv__(self, other):
        ...
        if np.isscalar(other):
            contains_zero = np.any(self.begin <= 0 & self.end >= 0)
            if contains_zero:
                raise ValueError('Division `other / self` not possible: '
                                 'Interval product contains 0'
                                 ''.format(other))
