...
void gridrec_v2_backproj( float *sino , int npix , int nang , float *angles , 
                          float *param , float *rec, char *fftwfn ) {
  ...
  calc_filter( filter_stand , nang , pdim , ctr , type_filter );
  ...
  for( n=0 ; n<nang ; n++ ){ 
    ...
    while( i < pdim ){
      cproj[j][0] = 0.0; //sino[ n * npix + npix - 1 ];
      cproj[j][1] = 0.0;
      i += 1;
      j += 1;
    }
	    ...
	    for( iv = vstart ; iv < vfin ; k++ , iv++ ) {
	      ...
	      // Select the centered square npix * npix
     	  ...
     	  if( padleft <= j && j < padright && padleft <= k && k < padright ){
	        ...
	        rec[index] = corrn * H[iu * pdim + iv][0];
	      }	  
	    }
